...
// Defining the calculated circuit design parameters output structure
// (Extern Variable Check Header File)
...
osc_condition osc_rc_phase_shift(osc_input_val* osc_params)
{
    ...
    if (osc_params->freq<0.1 || osc_params->freq>1000 || osc_params->Av<29 || osc_params->Av>100)
    {
            ...
            printf("\n !!! Note : The oscillating frequency range is from 0.1kHz to 1000kHz and amplitude gain range is from 1 to 100\n");
            return Limit_error;
    }
